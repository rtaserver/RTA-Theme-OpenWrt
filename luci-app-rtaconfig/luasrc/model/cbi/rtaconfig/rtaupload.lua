...
ful = SimpleForm('upload', translate('Upload  (Free: ') .. wa.byte_format(free_byte) .. ')', translate("You can upload jpg, png, and gif files to create your favorite rta desktop interface"))
ful.reset = false
ful.submit = false

sul = ful:section(SimpleSection, '', translate("Upload file to '/www/luci-static/rta/bg/'"))
fu = sul:option(FileUpload, '')
fu.template = 'rtaconfig/other_upload'
um = sul:option(DummyValue, '', nil)
um.template = 'rtaconfig/other_dvalue'

local dir, fd
dir = '/www/luci-static/rta/bg/'
nxfs.mkdir(dir)
http.setfilehandler(
    function(meta, chunk, eof)
        ...
        if eof and fd then
            fd:close()
            fd = nil
            um.value = translate('File saved to') .. ' "/www/luci-static/rta/bg/' .. meta.file .. '"'
        end
    end
)
